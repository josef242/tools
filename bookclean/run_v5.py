#!/usr/bin/env python3
"""Produce book.v5.jsonl: books3 front/back matter cut (ledgered), pg19
...
SRC = os.path.expanduser('~/data/book.v4.jsonl')
DST = os.path.expanduser('~/data/book.v5.jsonl')
SHARD_DIR = os.path.expanduser('~/data/.v5_shards')
...
def main():
    ...
    with open('reports/books3_clean_v5_ledger.jsonl', 'w') as L:
        for i in range(n):
