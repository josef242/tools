...
sys.path.insert(0, '.')
from books3_clean_v7 import clean_books3_v7
from midbook_clean import clean_midbook

V1 = os.path.expanduser('~/data/book.v1.jsonl')
V6 = os.path.expanduser('~/data/book.v6.jsonl')
DST = os.path.expanduser('~/data/book.v7.jsonl')
SHARDS = os.path.expanduser('~/data/.v7_shards')
...
def main():
    ...
    with open('reports/v7_ledger.jsonl','w') as L:
        for i in range(W):
