...
for k,txt in TEXT.items():
    ...
    for lo,hi in lb:
        if lo<=L<hi:
            ...
            if not pat and len(risk_examples[(lo,hi)])<6:
                risk_examples[(lo,hi)].append(f"x{FREQ[k]} {txt[:70]!r}")
            break
print(f"lines in >=100 books captured: {len(TEXT):,}\n")
print(f"{'len':<10}{'strong-pattern':>15}{'NO-pattern(risk)':>18}")
for lo,hi in lb:
    p,ng=tab[(lo,hi)]
    print(f"{lo}-{hi:<7}{p:>15,}{ng:>18,}")
print("\nNO-pattern lines (content-risk) by length — what a length floor would PROTECT:")
for lo,hi in lb:
    print(f"\n  len {lo}-{hi}:")
    for e in risk_examples[(lo,hi)]: print(f"    {e}")
